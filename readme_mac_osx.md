...
- `MOUSER_SIGN_IDENTITY` is required: a codesigning identity SHA-1 (list with `security find-identity -v -p codesigning`), normally set in `.env.local` (copy `.env.local.example`). Unset, empty, or `-` makes the script exit `2` before building; there is no ad-hoc fallback, because an ad-hoc signature changes on every rebuild and resets macOS Accessibility / Input Monitoring grants on every deploy. With an identity the script signs nested `.dylib` / `.so` / `.framework` files depth-first with `--options runtime`, then signs the outer bundle with `build_resources/Mouser.entitlements`, then runs `codesign --verify --deep --strict` and aborts the build if it fails. Stable permission behavior depends on unchanged source, resolved Python interpreter, dependencies, architecture, signing identity, entitlements, and timestamp policy.
- Check a machine's signing setup without building: `.venv/bin/python scripts/build_and_install.py --dry-run` prints the identity, version, install path, and whether the build would be routed through the GUI session.
- Building over SSH: the login keychain is unreachable there, so use `python3 scripts/build_macos_gui_session.py`, which routes the build through the console session (via deskflow's shared `tools/fleet-gui-exec.py` when `~/Desktop/deskflow` or `DESKFLOW_ROOT` has it, otherwise via Terminal.app). See DEVELOPMENT.md.
- This signed path is for local repeated developer builds. It is not a notarized release-signing workflow; a separate Developer ID signing, secure timestamp, notarization, stapling, and Gatekeeper assessment workflow does not exist yet.
