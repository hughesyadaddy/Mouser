...
Signing is driven by `MOUSER_SIGN_IDENTITY`, and it is required:

- It must be a codesigning identity SHA-1 (40 hex digits; list yours with `security find-identity -v -p codesigning`) set in `.env.local` (see `.env.local.example`). `scripts/build_and_install.py` loads `.env.local`; when invoking `./build_macos_app.sh` directly, export it yourself.
- Unset, empty, or `-`: both `build_macos_app.sh` and `scripts/build_and_install.py` print an error and exit `2` before running PyInstaller or stopping the installed app. There is no ad-hoc fallback. An ad-hoc signature (`codesign` with a lone `-` identity) is regenerated on every build, so macOS treats each build as a new app and resets its Accessibility / Input Monitoring grants; that is exactly the re-prompt-on-every-deploy behavior the fleet audit traced back to ad-hoc builds.
- `MOUSER_TEAM_ID` is a deprecated alias: when `MOUSER_SIGN_IDENTITY` is empty, `build_and_install.py` resolves the team's first keychain identity, validates it is a 40-hex hash, warns, and continues. Pin the hash instead.
- With a valid identity the script signs nested `.dylib` / `.so` / `.framework` files depth-first with `--options runtime`, then signs the outer bundle with the hardened-runtime exceptions at `build_resources/Mouser.entitlements` (`allow-jit`, `allow-unsigned-executable-memory`, `disable-library-validation`), then runs `codesign --verify --deep --strict --verbose=2` and aborts the build if verification fails. Permission grants stay stable across rebuilds as long as the source, resolved Python interpreter, dependency versions, architecture, signing identity, entitlements, and timestamp policy stay the same.

`scripts/build_and_install.py --dry-run` resolves the identity and prints the plan (identity hash, app version, build and install paths, whether GUI-session routing would be used) and exits `0` without building or installing anything; it exits `2` for a missing or ad-hoc identity, which makes it the cheapest way to check a seat's signing setup.

Building over SSH: codesign needs the login keychain, which an SSH session cannot reach (`errSecInternalComponent`). `scripts/build_macos_gui_session.py` detects that and hands the build to the console session. When the deskflow checkout is present (`DESKFLOW_ROOT`, default `~/Desktop/deskflow`) it delegates to the fleet-wide `deskflow/tools/fleet-gui-exec.py -- python3 scripts/build_and_install.py ...`, so Mouser and deskflow share one GUI-session route; otherwise it drives Terminal.app itself and waits for the sentinel. Neither path ever substitutes an ad-hoc identity.

Windows: after PyInstaller writes `dist/Mouser`, `build_and_install.py` runs `deskflow/scripts/sign-windows.ps1 -Root dist/Mouser` (same `DESKFLOW_ROOT` lookup) to Authenticode-sign every `.exe` / `.dll` with the certificate named by `DESKFLOW_SIGN_THUMBPRINT`. An unset thumbprint or a missing `signtool.exe` makes that script throw, and the build fails rather than installing an unsigned tree. The step is skipped only when the deskflow script itself is absent, and never runs on macOS.
...
The `MOUSER_SIGN_IDENTITY` path is not a notarized release-signing workflow. The release workflow (`.github/workflows/release.yml`) runs the same `build_macos_app.sh` and therefore also needs `MOUSER_SIGN_IDENTITY` provided from a secret; a separate Developer ID Application signing, secure timestamp, notarization, stapling, and Gatekeeper assessment workflow does not exist yet.
